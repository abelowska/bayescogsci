...
transformed parameters {
  ...
  for (n in 1:N_obs){
    real a = inv_logit(alpha[1] + u[subj[n], 1]);
    real c = inv_logit(alpha[2] + u[subj[n], 2]);
    real t = inv_logit(alpha[3] + u[subj[n], 3]);
    real f = inv_logit(alpha[4] + u[subj[n], 4] + complexity[n] * beta_f);
...
model {
  ...
  target += normal_lpdf(tau | 0, 1)  - 4 * normal_lccdf(0 | 0, 1);
